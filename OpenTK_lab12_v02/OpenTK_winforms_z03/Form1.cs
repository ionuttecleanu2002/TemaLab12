    ...
    public partial class Form1 : Form {
        ...
        private int[,] loadVertexVbo()
        {
            ...
            try
            {
                ...
                int[,] arrVertexer = new int[nVertexer, 3];
                Console.WriteLine("Vertexuri citite: " + nVertexer.ToString());
        ...
        private void GlControl1_Paint(object sender, PaintEventArgs e) {
            ...
            if (VBOon == true)
            {
                ...
                Console.WriteLine("VBO test = " + VBOobject.ToString());
            }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            int[,] arrVertexer = null;      // atentie!!!

            // Citește vertexurile - manual
            try
            {
                StreamReader fileReader = new StreamReader(("vertexDrawVbo.txt"));
                int nVertexer = Convert.ToInt32(fileReader.ReadLine().Trim());
            ...
            // Construiește obiectul de coordonate
            Vector3[] vertexObj = new Vector3[nVert];    // fară verificare de eroare!
            Console.WriteLine("Nr. de linii VBO:" + nVert);
            for (int i = 0; i < nVert; i++)
            {
                Console.WriteLine("Linia este: " + i + " --- (" + arrVertexer[i, 0] + "," + arrVertexer[i, 1] + "," + arrVertexer[i, 2] + ")");
                Console.WriteLine("");
            ...
            // inițiază obiectul VBO
            GL.BindTexture(TextureTarget.Texture2D, SpecialTexture.id);
            VBOobject = GL.GenBuffer();
            GL.BindBuffer(BufferTarget.ArrayBuffer, VBOobject);
            GL.BufferData<Vector3>(BufferTarget.ArrayBuffer, (IntPtr)(Vector3.SizeInBytes * vertexObj.Length), vertexObj, BufferUsageHint.StaticDraw);  // se referă la obiectul curent activ/binded!!!

            // enable drawing of vertex buffer object
            VBOon = true;
